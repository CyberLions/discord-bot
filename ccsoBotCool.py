...
'''TODO add functionality in main
    ...
    maybe post a gif
    flag = w00_b4ck_b4by'''
async def youCannotSayPopAndForgetTheSmoke(message): #For Josh
...
async def capybaraTime(message): #For Zach
        capybaras = ["https://i.imgur.com/lEy6WEe.jpeg",
        ...
        await message.channel.send(capybaraCool)

async def vimOnACube(message): #For Eugene
    cubeLink = "https://github.com/oakes/vim_cubed"
    await message.channel.send("It's on a cube! " + cubeLink)

async def simpy(message): #As a reminder for and to Petr
    simpy = "simpy.png"
